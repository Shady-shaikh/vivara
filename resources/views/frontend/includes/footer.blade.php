            ...
            <div class="col-lg-2">
                ...
                <ul>
                    ...
                    @foreach ($cmspages as $row)
                        {{-- @if ($row->column_type == 'quick_links' && $row->show_hide && $row->cms_pages_footer) --}}
                        @if ($row->show_hide && $row->cms_pages_footer && empty($row->cms_pages_link))
                            <li><a href="{{ route('pages.index', ['id' => $row->cms_slug]) }}">
                                    <i class="fal fa-chevron-right"></i>
                                    <span>{{ $row->cms_pages_title }}</span></a></li>
                        @else
                        <li><a href="{{ $row->cms_pages_link }}" target="_blank">
                          <i class="fal fa-chevron-right"></i>
                          <span>{{ $row->cms_pages_title }}</span></a></li>
                        @endif
                    @endforeach
            ...
            <div class="col-lg-4">
                ...
                <div class="row g-5">
                    <div class="col-lg-6">
                        <ul>
                            {{-- {{dd($uniqueCategories)}} --}}
                            @foreach ($uniqueCategories as $row)
                                <li>
                                    <a href="{{ route('products.index', ['id' => $row]) }}">
                                        <i class="fal fa-chevron-right"></i>
                                        <span>{{ $row }}</span>
                                    </a>
                                </li>
...
<script src="{{ asset('public/frontend-assets/js/lightbox-plus-jquery.min.js') }}"></script>
...
<script>
    ...
    $('.add_cart').click(function(event) {
        ...
        var url = "{{ route('cart.store', ['id' => ':idValue']) }}";
...
<script>
    $(document).ready(function() {
        ...
        $('#shipping_charges').change(function() {
            ...
            var url = "{{ route('products.getshipcharge') }}"; // Example: '/retrieve-shipping-data'
