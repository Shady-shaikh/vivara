    ...
    <div class="container">
      <div class="row d-flex justify-content-center text-center g-0">
        <div class=" adminpanel-right text-start login-page">
          ...
          <form class="login-form card card-md form" action="{{ route('user.auth') }}" method="POST">
            <h2 class="heading card-title text-center">Login to your account</h1>
              <div class="card-body">
                ...
                <a href="{{route('user.dashboard')}}" style="text-decoration: none;margin-left: 14px;background: #ccc;padding: 9px;color: black;">Back</a>
                <p class="message">Not registered? <a href="{{ url('/user/register') }}">Create an account</a></p>
                <a class="message" href="{{ route('user.forgot_password') }}">Forgot Password</a>
              </div>
          </form>
                {{-- {{dd(request()->toArray())}} --}}
        </div>
