    ...
    <div class="content-header row">
        <div class="py-4">
            ...
            <div class="d-flex justify-content-between w-100 flex-wrap">
                ...
                <div>
                    @if (in_array('Create Apply Offers', $permissions))
                        <a href="{{ url('admin/offers/create') }}"
                                    ...
                                    <tbody>
                                        @foreach ($model as $ind => $item)
                                            <tr>
                                                ...
                                                <td>{{ $ind + 1 }}</td>
                                                <td>{{ $item->name }}</td>
                                                <td> {{ $data->scheme_title ?? '' }}</td>
                                                <td>
                                                    @if (in_array('Update Apply Offers', $permissions))
                                                        <a href="{{ route('admin.offers.edit', ['id' => $item->item_id]) }}"
                                                            class="btn btn-primary "><i class="fa-solid fa-pencil"></i></a>
                                                    @endif
                                                    @if (in_array('Delete Apply Offers', $permissions))
                                                        {!! Form::open([
                                                            'method' => 'GET',
